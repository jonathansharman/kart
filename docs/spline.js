...
export class SplineCorner {
    constructor(vertex, smoothness) {
        this.vertex = vertex;
        this.smoothness = smoothness;
    }
}
export const corner = (x, y, smoothness) => {
...
export class SplineLoop {
    ...
    // individual lengths).
    at(t) {
        const idx = Math.floor(t * this.sections.length) % this.sections.length;
        const sectionT = (t * this.sections.length) % 1;
    ...
    containsPoint(p, samplesPerSection = 100) {
    ...
    pointIsWithinDistance(p, distance) {
        for (let section of this.sections) {
